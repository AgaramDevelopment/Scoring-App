...
(NSString *)WICKETEVENT
{
    ...
    if(![DBManagerInsertScoreEngine GetBallCodeForInsertScoreEngine : COMPETITIONCODE :MATCHCODE ])
    {
        [DBManagerInsertScoreEngine UpdateMatchStatusForInsertScoreEngine : COMPETITIONCODE :MATCHCODE ];
    ...
    if(![DBManagerInsertScoreEngine GetBallCodesForInsertScoreEngine : COMPETITIONCODE :MATCHCODE :TEAMCODE: INNINGSNO:OVERNO: BALLNO : BALLCOUNT ])
    {
        MAXID=[DBManagerInsertScoreEngine GetMaxIdForInsertScoreEngine :MATCHCODE ];
        
        
        //BALLCODENO = MATCHCODE + RIGHT(REPLICATE('0',10)+CAST(MAXID AS VARCHAR(10)),10);
        
        if(INSERTTYPE.length >0)
        {
            NSMutableArray *GetInsertTypeGreaterthanZeroDetails=[ DBManagerInsertScoreEngine GetInsertTypeGreaterthanZeroDetailsForInsertScoreEngine :  COMPETITIONCODE:  MATCHCODE: BALLCODE ];
            if(GetInsertTypeGreaterthanZeroDetails.count>0)
            {
                
                INNINGSNO=[GetInsertTypeGreaterthanZeroDetails objectAtIndex:0];
                TEAMCODE=[GetInsertTypeGreaterthanZeroDetails objectAtIndex:1];
                T_OVERNO=[GetInsertTypeGreaterthanZeroDetails objectAtIndex:2];
                T_BALLNO=[GetInsertTypeGreaterthanZeroDetails objectAtIndex:3];
                T_BALLCOUNT=[GetInsertTypeGreaterthanZeroDetails objectAtIndex:4];
                T_ISLEGALBALL=[GetInsertTypeGreaterthanZeroDetails objectAtIndex:5];
                SB_STRIKERCODE=[GetInsertTypeGreaterthanZeroDetails objectAtIndex:6];
                SB_BOWLERCODE=[GetInsertTypeGreaterthanZeroDetails objectAtIndex:7];
            ...
            if (INSERTTYPE == @"BEFORE")
            {
                ...
                T_BALLCOUNT = [NSNumber numberWithInt:1];
            ...
            [DBManagerInsertScoreEngine UpdateBallEventtablesForInsertScoreEngine : COMPETITIONCODE :MATCHCODE :TEAMCODE :INNINGSNO :TEAMCODE: T_OVERNO : T_BALLNO : T_BALLCOUNT];
        }
    ...
    else if(	INSERTTYPE == @"AFTER")
    {
        ...
        T_ISLEGALBALL = [NSNumber numberWithInt:1];
    ...
    else if([DBManagerInsertScoreEngine GetBallCodeForUpdateBowlerOverDetailsForInsertScoreEngine :COMPETITIONCODE: MATCHCODE:TEAMCODE:INNINGSNO : OVERNO])
    {
