...
+(BOOL)checkExpiryDate: (NSString *) userId{
    ...
    if(retVal ==0){
        ...
        NSString *query=[NSString stringWithFormat:@"SELECT  *  FROM Userdetails WHERE  strftime('%%Y-%%m-%%d-', LICENSEUPTO) >= CURRENT_DATE AND USERCODE = '%@'",userId];
        NSLog(@"%@",query);
        stmt=[query UTF8String];
        if(sqlite3_prepare(dataBase, stmt, -1, &statement, NULL)==SQLITE_OK)
        {
...
+(BOOL)checkSecurityExpiryDate: (NSString *) USERNAME{
    ...
    if(retVal ==0){
        ...
        NSString *query=[NSString stringWithFormat:@"SELECT  *  FROM SECUREIDDETAILS WHERE  strftime('%%Y-%%m-%%d-', ENDDATE) >= CURRENT_DATE AND USERNAME = '%@'",USERNAME];
        NSLog(@"%@",query);
        stmt=[query UTF8String];
        if(sqlite3_prepare(dataBase, stmt, -1, &statement, NULL)==SQLITE_OK)
        {
