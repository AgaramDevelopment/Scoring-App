...
-(BOOL)checkExpiryDate: (NSString *) userId{
    ...
    if (sqlite3_open([dbPath UTF8String], &dataBase) == SQLITE_OK)
    {
        ...
        NSString *query=[NSString stringWithFormat:@"SELECT  USERCODE  FROM Userdetails WHERE  strftime('%%Y-%%m-%%d %%H:%%M:%%S', LICENSEUPTO) >= datetime('now') AND USERCODE = '%@'",userId];
        NSLog(@"%@",query);
        stmt=[query UTF8String];
        if(sqlite3_prepare(dataBase, stmt, -1, &statement, NULL)==SQLITE_OK)
        {
...
-(BOOL)checkSecurityExpiryDate: (NSString *) USERNAME{
    ...
    if (sqlite3_open([dbPath UTF8String], &dataBase) == SQLITE_OK)
    {
        ...
        NSString *query=[NSString stringWithFormat:@"SELECT  USERNAME  FROM SECUREIDDETAILS WHERE  strftime('%%Y-%%m-%%d %%H:%%M:%%S', ENDDATE) >= datetime('now') AND USERNAME = '%@'",USERNAME];
        NSLog(@"%@",query);
        stmt=[query UTF8String];
        if(sqlite3_prepare(dataBase, stmt, -1, &statement, NULL)==SQLITE_OK)
        {
...
-(NSString *) getPenaltyScore:(NSString*)COMPETITIONCODE:(NSString*) MATCHCODE:(NSString*)INNINGSNO:(NSString*)BATTINGTEAMCODE{
    ...
    if (sqlite3_open([dbPath UTF8String], &dataBase) == SQLITE_OK)
    {
        ...
        NSString *query = [NSString stringWithFormat:@"SELECT IFNULL(SUM(PENALTYRUNS),0)FROM PENALTYDETAILS WHERE COMPETITIONCODE = '%@' AND MATCHCODE = '%@' AND INNINGSNO IN ('%@', '%@' - 1)AND AWARDEDTOTEAMCODE = '%@' AND (BALLCODE ISNULL  OR BALLCODE IS '(null)' OR INNINGSNO = '%@' - 1)",COMPETITIONCODE,MATCHCODE, INNINGSNO,INNINGSNO,BATTINGTEAMCODE,INNINGSNO];
        stmt=[query UTF8String];
