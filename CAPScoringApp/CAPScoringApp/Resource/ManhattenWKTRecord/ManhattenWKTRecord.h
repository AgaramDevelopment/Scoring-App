//
//  ManhattenWKTRecord.h
//  CAPScoringApp
//
//  Created by APPLE on 31/10/16.
//  Copyright © 2016 agaram. All rights reserved.
//

#import <Foundation/Foundation.h>

@interface ManhattenWKTRecord : NSObject

@property (nonatomic,strong) NSString * inningno;

@property (nonatomic,strong) NSString * batsmancode;

@property (nonatomic,strong) NSString * bowlercode;

@property (nonatomic,strong) NSString * batsmanname;

@property (nonatomic,strong) NSString * Run;

@property (nonatomic,strong) NSString * balls;

@property (nonatomic,strong) NSString * bowlername;

@property (nonatomic,strong) NSString * wicketdescription;

@property (nonatomic,strong) NSString * wicketno;

@property (nonatomic,strong) NSString * wicketballno;

@property (nonatomic,strong) NSString * wicketscore;

@property (nonatomic,strong) NSString * wickettypedescription;

@property (nonatomic,strong) NSString * wicketoverno;

@property (nonatomic,strong) NSNumber * tag;



@end
