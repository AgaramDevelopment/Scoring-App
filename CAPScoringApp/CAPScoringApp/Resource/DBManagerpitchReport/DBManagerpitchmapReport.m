...
-(NSMutableArray *)getPitchmapdetails:(NSString *) MATCHTYPECODE :(NSString *) COMPETITIONCODE:(NSString *)MATCHCODE:(NSString *) TEAMCODE :(NSString *) INNINGSNO:(NSString *) SESSIONS:(NSString *) DAYS:(NSString *) STRIKER:(NSString *) NONSTRIKER:(NSString *) BOWLER :(NSString *) SHOT:(NSString *)REGION:(NSString *) RUNS:(NSString *) BOUNFOUR:(NSString *) BOUNSIX:(NSString *) WIDE:(NSString *)NOBALL:(NSString *) BYES:(NSString *) LEGBYES:(NSString *) ISAPPEAL:(NSString *) ISUNCOMFORATABLE:(NSString *) ISBEATEN:(NSString *) ISWICKETTAKINGBALL:(NSString *) ISRELEASESHOT:(NSString *)FROMOVER:(NSString *) TOOVER:(NSString *) SHOTTYPECATEGORY:(NSString *) BATTINGSTYLE:(NSString *)BOWLINGSTYLE:(NSString *) BOWLINGTYPE:(NSString *) WICKETTYPE:(NSString *) BOWLINGSPEC:(NSString *)STARTDATE:(NSString *) ENDDATE:(NSString *) VENUE:(NSString *) DEFAULT
{
    NSMutableArray *eventArray=[[NSMutableArray alloc]init];
    ...
    if (sqlite3_open([dbPath UTF8String], &dataBase) == SQLITE_OK)
    {
        ...
        NSString *query=[NSString stringWithFormat:@"SELECT BALL.PMLINECODE, MDLIN.METASUBCODEDESCRIPTION PMLINENAME, BALL.PMLENGTHCODE, MDLEN.METASUBCODEDESCRIPTION PMLENGTHNAME, BALL.PMX2, BALL.PMY2, BALL.RUNS+BALL.OVERTHROW AS RUNS, STKR.BATTINGSTYLE, (CASE WHEN (BALL.RUNS + (CASE WHEN BALL.BYES = 0 AND BALL.LEGBYES = 0 THEN BALL.OVERTHROW ELSE 0 END) + BALL.WIDE) = 0 THEN 1 ELSE 0 END) ISDOTBALL, (CASE WHEN WKT.WICKETTYPE IS NULL THEN 0 ELSE 1 END) ISWICKET, WKT.WICKETTYPE, (MRM.VIDEOLOCATION + '\' + BALL.VIDEOFILENAME) VIDEOFILEPATH, dbo.InitCap(('MATCH: '+MRM.MATCHNAME)) AS MATCHNAME, dbo.InitCap(('BALL NO: '+CAST(BALL.OVERNO AS NVARCHAR(MAX))+'.'+CAST(BALL.BALLNO AS NVARCHAR(MAX)))) AS BALLNO, dbo.InitCap((PM1.PLAYERNAME+' TO '+STKR.PLAYERNAME)) AS PLAYERDETAIL, dbo.InitCap('LINE: '+ME.METASUBCODEDESCRIPTION) AS LINE, dbo.InitCap('LENGTH: '+ME1.METASUBCODEDESCRIPTION) AS LENGTH, dbo.InitCap('BOWL TYPE: '+ME.METASUBCODEDESCRIPTION) AS LINETYPE, dbo.InitCap('SHOT TYPE: '+ME1.METASUBCODEDESCRIPTION) AS SHOTYPE, dbo.InitCap('RUNS: '+CAST(BALL.RUNS+BALL.OVERTHROW AS NVARCHAR(MAX))) AS BALLRUNS,  CASE   WKT.WICKETTYPE WHEN 'MSC095' THEN 'c ' + FLDRC.PLAYERNAME + ' b ' + PM1.PLAYERNAME WHEN 'MSC096' THEN 'b ' + PM1.PLAYERNAME WHEN 'MSC097' THEN 'run out ' + FLDRC.PLAYERNAME WHEN 'MSC104' THEN 'st ' + FLDRC.PLAYERNAME + ' b '+ PM1.PLAYERNAME WHEN 'MSC098' THEN 'lbw ' + PM1.PLAYERNAME WHEN 'MSC099' THEN 'hit wicket' +' '+ PM1.PLAYERNAME WHEN 'MSC100' THEN 'Handled the ball' WHEN 'MSC105' THEN 'c & b' +' '+ PM1.PLAYERNAME WHEN 'MSC101' THEN 'Timed Out' WHEN 'MSC102' THEN 'Retired Hurt' WHEN 'MSC103' THEN 'Hitting Twice' WHEN 'MSC107' THEN 'Mankading' WHEN 'MSC108' THEN 'Retired Out' WHEN 'MSC106' THEN 'Obstructing the field' WHEN 'MSC133' THEN 'Absent Hurt' ELSE   'Not Out' END AS WICKETTYPE FROM    BALLEVENTS BALL INNER JOIN MATCHREGISTRATION MRM ON MRM.MATCHCODE = BALL.MATCHCODE AND MRM.RECORDSTATUS='MSC001' INNER JOIN COMPETITION CM ON CM.COMPETITIONCODE=MRM.COMPETITIONCODE  AND CM.RECORDSTATUS='MSC001' INNER JOIN PLAYERMASTER STKR ON BALL.STRIKERCODE = STKR.PLAYERCODE AND STKR.RECORDSTATUS='MSC001' LEFT JOIN METADATA MDLIN ON BALL.PMLINECODE = MDLIN.METASUBCODE LEFT JOIN METADATA MDLEN ON BALL.PMLENGTHCODE = MDLEN.METASUBCODE LEFT JOIN WICKETEVENTS WKT ON BALL.COMPETITIONCODE = WKT.COMPETITIONCODE AND BALL.MATCHCODE = WKT.MATCHCODE AND BALL.INNINGSNO = WKT.INNINGSNO AND BALL.TEAMCODE = WKT.TEAMCODE AND BALL.BALLCODE = WKT.BALLCODE INNER JOIN PLAYERMASTER PM1 ON BALL.BOWLERCODE = PM1.PLAYERCODE AND PM1.RECORDSTATUS='MSC001' LEFT JOIN  METADATA ME ON ME.METASUBCODE=BALL.PMLINECODE LEFT JOIN  METADATA ME1 ON ME1.METASUBCODE=BALL.PMLENGTHCODE LEFT JOIN PLAYERMASTER FLDRC   ON FLDRC.PLAYERCODE = WKT.FIELDINGPLAYER AND FLDRC.RECORDSTATUS='MSC001' WHERE  ('%@'='' OR CM.MATCHTYPE IN (SELECT ID FROM DBO.SPLIT('%@'))) AND ('%@'='' OR BALL.COMPETITIONCODE IN (SELECT ID FROM DBO.SPLIT('%@'))) AND ('%@'='' OR BALL.MATCHCODE IN (SELECT ID FROM DBO.SPLIT('%@'))) AND ('%@'='' OR BALL.TEAMCODE IN(SELECT ID FROM DBO.SPLIT('%@'))) AND (CAST('%@' AS NVARCHAR)='' OR CAST(BALL.INNINGSNO AS NVARCHAR) IN(SELECT ID FROM DBO.SPLIT('%@'))) AND ('%@'='' OR CAST(BALL.DAYNO AS NVARCHAR) IN(SELECT ID FROM DBO.SPLIT('%@'))) AND ('%@'='' OR CAST(BALL.SESSIONNO AS NVARCHAR) IN(SELECT ID FROM DBO.SPLIT('%@'))) AND ('%@'='' OR BALL.STRIKERCODE IN(SELECT ID FROM DBO.SPLIT('%@'))) AND ('%@'='' OR BALL.NONSTRIKERCODE IN(SELECT ID FROM DBO.SPLIT('%@'))) AND ('%@'='' OR BALL.BOWLERCODE IN(SELECT ID FROM DBO.SPLIT('%@'))) AND ('%@'='' OR BALL.SHOTTYPE IN(SELECT ID FROM DBO.SPLIT('%@'))) AND ('%@'='' OR BALL.WWREGION IN(SELECT ID FROM DBO.SPLIT('%@'))) AND ('%@'=''OR ((CAST(BALL.RUNS AS NVARCHAR) IN(SELECT ID FROM DBO.SPLIT('%@'))  AND BALL.ISFOUR=0 AND BALL.ISSIX=0) OR ('%@' = '1' AND BALL.ISFOUR = 1) OR ('%@' = '1' AND BALL.ISSIX = 1))) AND ('%@' = -1 OR BALL.OVERNO >= '%@') AND ('%@' = -1 OR BALL.OVERNO  <= '%@') AND ( ('%@' = '0' AND '%@' = '0' AND '%@'='0' AND '%@'='0' ) OR ('%@' = '1' AND BALL.WIDE > 0 ) OR ('%@' = '1' AND BALL.NOBALL > 0) OR ('%@' = '1' AND BALL.BYES > 0) OR ('%@' = '1' AND BALL.LEGBYES > 0)) AND (( '%@'='0' AND '%@'='0' AND '%@'='0' AND '%@'='0' AND '%@'='0') OR ('%@' = '1' AND CAST(BALL.ISAPPEAL AS NVARCHAR) = '%@') OR ('%@' = '1' AND CAST(BALL.ISBEATEN AS NVARCHAR) = '%@') OR ('%@' = '1' AND CAST(BALL.ISWTB AS NVARCHAR) = '%@') OR ('%@' = '1' AND CAST(BALL.ISUNCOMFORT AS NVARCHAR) = '%@') OR ('%@' = '1' AND CAST(BALL.ISRELEASESHOT AS NVARCHAR) = '%@')) AND ('%@'='' OR BALL.WIDE= 0) AND ( '%@'='' OR STKR.BATTINGSTYLE  IN (SELECT ID FROM DBO.SPLIT('%@'))) AND ( '%@'='' OR PM1.BOWLINGSTYLE  IN (SELECT ID FROM DBO.SPLIT('%@'))) AND ( '%@'='' OR PM1.BOWLINGTYPE  IN (SELECT ID FROM DBO.SPLIT('%@'))) AND ( '%@' = NULL OR '%@'='' OR BALL.SHOTTYPECATEGORY  IN (SELECT ID FROM DBO.SPLIT('%@'))) AND ( '%@' = NULL OR '%@'='' OR WKT.WICKETTYPE  IN (SELECT ID FROM DBO.SPLIT('%@'))) AND ( '%@'='' OR PM1.BOWLINGSPECIALIZATION IN (SELECT ID FROM DBO.SPLIT('%@'))) AND ('%@' ='%@' OR CAST(MRM.MATCHDATE AS DATE) >=CAST('%@' AS DATE)) AND ('%@' ='%@' OR CAST(MRM.MATCHDATE AS DATE) <=CAST('%@' AS DATE)) AND ('%@' =''OR MRM.GROUNDCODE IN (SELECT ID FROM DBO.SPLIT('%@'))) ",MATCHTYPECODE,MATCHTYPECODE,COMPETITIONCODE,COMPETITIONCODE,MATCHCODE,MATCHCODE,TEAMCODE,TEAMCODE,INNINGSNO,INNINGSNO,DAYS,DAYS,SESSIONS,SESSIONS,STRIKER,STRIKER,NONSTRIKER,NONSTRIKER,BOWLER,BOWLER,SHOT,SHOT,REGION,REGION,RUNS,RUNS,BOUNFOUR,BOUNSIX,FROMOVER,FROMOVER,TOOVER,TOOVER,WIDE,NOBALL,BYES,LEGBYES,WIDE,NOBALL,BYES,LEGBYES,ISAPPEAL,ISBEATEN,ISWICKETTAKINGBALL,ISUNCOMFORATABLE,ISRELEASESHOT,ISAPPEAL,ISAPPEAL,ISBEATEN,ISBEATEN,ISWICKETTAKINGBALL,ISWICKETTAKINGBALL,ISUNCOMFORATABLE,ISUNCOMFORATABLE,ISRELEASESHOT,ISRELEASESHOT,STRIKER,BATTINGSTYLE,BATTINGSTYLE,BOWLINGSTYLE,BOWLINGSTYLE,BOWLINGTYPE,BOWLINGTYPE,SHOTTYPECATEGORY,SHOTTYPECATEGORY,SHOTTYPECATEGORY,WICKETTYPE,WICKETTYPE,WICKETTYPE,BOWLINGSPEC,BOWLINGSPEC,STARTDATE,DEFAULT,STARTDATE,ENDDATE,DEFAULT,ENDDATE,VENUE,VENUE];
