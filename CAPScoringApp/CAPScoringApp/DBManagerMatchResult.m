...
+(NSMutableArray *) GetManOfTheMatchDetailsForFetchMatchResult :(NSString*) MATCHCODE{
    ...
    if (sqlite3_open(dbPath, &dataBase) == SQLITE_OK)
    {
        NSString *updateSQL = [NSString stringWithFormat:@"SELECT MTP.PLAYERCODE AS  PLAYERCODE,PM.PLAYERNAME FROM MATCHTEAMPLAYERDETAILS MTP INNER JOIN PLAYERMASTER PM ON PM.PLAYERCODE=MTP.PLAYERCODE WHERE  MTP.MATCHCODE='%@' GROUP BY MTP.PLAYERCODE,PM.PLAYERNAME",MATCHCODE];
