...
@interface EditModeVC ()
{
    ...
    UIButton *Btn_Indexpath;
    NSMutableArray * objSelectBallCodeArray;
    NSString*  seleBtnIndex;
}
...
{
    NSString *SelectBallCode;
    int SelectBall =[selectIndex intValue];
    
    for (int i=0; i < objSelectBallCodeArray.count; i++)
    {
        if(SelectBall == i)
        {
            SelectBallCode=[objSelectBallCodeArray objectAtIndex:i];
        }
    }
