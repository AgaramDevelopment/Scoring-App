...
-(NSMutableArray *) GetBowlingSummaryInElseForScoreBoard:(NSString*) COMPETITIONCODE:(NSString*) MATCHCODE :(NSString*) INNINGSNO
{
    ...
    if (sqlite3_open(dbPath, &dataBase) == SQLITE_OK)
    {
        NSString *updateSQL = [NSString stringWithFormat:@"SELECT BOWLERCODE, BOWLERNAME, TOTALMINUTES =''  , CASE WHEN PARTIALOVERBALLS > 0       THEN CAST(((((OVERS * 6.0) + BALLS) / 6.0) + ((((OVERS * 6.0) + BALLS) %% 6)/10.0)) AS NUMERIC(6,1))       ELSE CAST((OVERS + (BALLS/10.0)) AS NUMERIC(6,1))      END OVERS    , MAIDENS, RUNS, WICKETS, NOBALLS, WIDES, DOTBALLS, FOURS, SIXES    , CAST((CASE WHEN ((OVERS * 6) + BALLS) = 0 THEN 0.0 ELSE ((RUNS / ((OVERS * 6.0) + BALLS)) * 6) END) AS NUMERIC (6,2)) ECONOMY    FROM    (SELECT BS.BOWLINGPOSITIONNO, BS.BOWLERCODE, BC.PLAYERNAME BOWLERNAME,  julianday(BIOT.STARTTIME) - julianday(BIOT.ENDTIME)  TOTALMINUTES     , BS.OVERS     , (BS.BALLS + BS.PARTIALOVERBALLS) BALLS     , BS.PARTIALOVERBALLS     , BS.MAIDENS , BS.RUNS, BS.WICKETS, BS.NOBALLS, BS.WIDES, BS.DOTBALLS, BS.FOURS, BS.SIXES     FROM BOWLINGSUMMARY BS     INNER JOIN PLAYERMASTER BC     ON BC.PLAYERCODE = BS.BOWLERCODE     LEFT JOIN BOWLEROVERDETAILS BIOT     ON BIOT.BOWLERCODE = BS.BOWLERCODE     AND BIOT.COMPETITIONCODE = BS.COMPETITIONCODE     AND BIOT.MATCHCODE = BS.MATCHCODE     AND BIOT.INNINGSNO = BS.INNINGSNO     WHERE BS.COMPETITIONCODE = '%@'     AND BS.MATCHCODE ='%@' AND BS.INNINGSNO ='%@'     GROUP BY BS.BOWLINGPOSITIONNO, BS.BOWLERCODE, BC.PLAYERNAME, BS.OVERS, BS.BALLS, BS.PARTIALOVERBALLS, BS.MAIDENS , BS.RUNS,BS.WICKETS, BS.NOBALLS, BS.WIDES, BS.DOTBALLS, BS.FOURS, BS.SIXES    ) BOWLINGCARD    ORDER BY BOWLINGPOSITIONNO  ",COMPETITIONCODE,MATCHCODE,INNINGSNO];
