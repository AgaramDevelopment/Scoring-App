...

{
    ...
    if (sqlite3_open(dbPath, &dataBase) == SQLITE_OK)
    {
        NSString *updateSQL = [NSString stringWithFormat:@"SELECT BS.COMPETITIONCODE, BS.MATCHCODE, BS.BATTINGTEAMCODE, BS.INNINGSNO, BS.BATSMANCODE, BS.RUNS, BS.BALLS, BS.ONES, BS.TWOS, BS.THREES, BS.FOURS, BS.SIXES, BS.DOTBALLS, BS.WICKETNO, BS.WICKETTYPE, BS.FIELDERCODE, BS.BOWLERCODE, BS.WICKETOVERNO, BS.WICKETBALLNO, BS.WICKETSCORE,BS.BATTINGPOSITIONNO,(SELECT COUNT(1) FROM (SELECT BE.MATCHCODE,INNINGSNO,PLAYERMASTER.PLAYERNAME, BE.STRIKERCODE, (MIN(BE.OVERNO || '.' || BE.BALLNO || BE.BALLCOUNT) - 0.01) OVERBALL FROM BALLEVENTS BE INNER JOIN PLAYERMASTER PLAYERMASTER ON PLAYERMASTER.PLAYERCODE = BE.STRIKERCODE WHERE BE.MATCHCODE = '%@' AND TEAMCODE =	'%@' AND INNINGSNO = '%@' GROUP BY BE.MATCHCODE,INNINGSNO, STRIKERCODE, PLAYERMASTER.PLAYERNAME UNION SELECT BE.MATCHCODE,INNINGSNO,PLAYERMASTER.PLAYERNAME, BE.NONSTRIKERCODE , (MIN(BE.OVERNO || '.' || BE.BALLNO || BE.BALLCOUNT) - 0.01) OVERBALL FROM BALLEVENTS BE INNER JOIN PLAYERMASTER PLAYERMASTER ON PLAYERMASTER.PLAYERCODE = BE.NONSTRIKERCODE WHERE BE.MATCHCODE = '%@' AND TEAMCODE =	'%@' AND INNINGSNO = '%@' GROUP BY BE.MATCHCODE,BE.INNINGSNO, BE.NONSTRIKERCODE, PLAYERMASTER.PLAYERNAME) SAMSUB WHERE SAMSUB.MATCHCODE = SAM.MATCHCODE AND SAMSUB.INNINGSNO = SAM.INNINGSNO AND SAMSUB.OVERBALL<= SAM.OVERBALL) BATTINGPOSITIONNO FROM (SELECT BE.MATCHCODE,INNINGSNO,PLAYERMASTER.PLAYERNAME, STRIKERCODE, (MIN(BE.OVERNO || '.' || BE.BALLNO || BE.BALLCOUNT) - 0.01) OVERBALL FROM BALLEVENTS BE INNER JOIN PLAYERMASTER PLAYERMASTER ON PLAYERMASTER.PLAYERCODE = BE.STRIKERCODE WHERE BE.MATCHCODE = '%@' AND TEAMCODE =	'%@' AND INNINGSNO = %@ GROUP BY BE.MATCHCODE,INNINGSNO, STRIKERCODE, PLAYERMASTER.PLAYERNAME UNION SELECT BE.MATCHCODE,INNINGSNO,PLAYERMASTER.PLAYERNAME, BE.NONSTRIKERCODE , (MIN(BE.OVERNO || '.' || BE.BALLNO || BE.BALLCOUNT) - 0.01) OVERBALL FROM BALLEVENTS BE INNER JOIN PLAYERMASTER PLAYERMASTER ON PLAYERMASTER.PLAYERCODE = BE.NONSTRIKERCODE WHERE BE.MATCHCODE = '%@' AND TEAMCODE =	'%@' AND INNINGSNO = %@ GROUP BY BE.MATCHCODE,BE.INNINGSNO, BE.NONSTRIKERCODE, PLAYERMASTER.PLAYERNAME) SAM INNER JOIN BATTINGSUMMARY BS ON SAM.MATCHCODE = BS.MATCHCODE AND SAM.INNINGSNO = BS.INNINGSNO AND SAM.STRIKERCODE = BS.BATSMANCODE GROUP by BS.BATSMANCODE ORDER by BS.BATTINGPOSITIONNO", MATCHCODE,TEAMCODE,INNINGSNO, MATCHCODE,TEAMCODE,INNINGSNO,MATCHCODE,TEAMCODE,INNINGSNO,MATCHCODE,TEAMCODE,INNINGSNO];
