    ...
    if (sqlite3_open(dbPath, &dataBase) == SQLITE_OK)
    {
        ...
        if(sqlite3_prepare_v2(dataBase, update_stmt,-1, &statement, NULL)==SQLITE_OK){
            ...
            if (sqlite3_step(statement) == SQLITE_DONE)
            {
                                ...
                                record.PMX2=[self getValueByNull:statement :2];
                record.PMy2=[self getValueByNull:statement :3];
                record.BattingStyle=[self getValueByNull:statement :4];
                record.Runs=[self getValueByNull:statement :5];
                record.isDontBall=[self getValueByNull:statement :6];
                record.IsWicket=[self getValueByNull:statement :7];
                record.IsonSide=[self getValueByNull:statement :8];
                                [GetBowlingTDetails addObject:record];
            }
            sqlite3_reset(statement);
